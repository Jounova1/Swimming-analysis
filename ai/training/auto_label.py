...
# Minimum box area as fraction of frame (filter tiny/distant detections)
MIN_BOX_AREA = 0.002


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Auto-label swimmers by detecting only person class with YOLOv11."
    )
    parser.add_argument(
        "--image-folder",
        type=Path,
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="yolov11x.pt",  # FIX 1: use largest model for auto-labeling accuracy
        help="Path to YOLOv8 model weights.",
    )
    parser.add_argument(
        "--conf",
        type=float,
        default=0.45,  # FIX 2: raised from 0.15 — only keep high-confidence labels
        help="Confidence threshold for detections.",
    )
    parser.add_argument(
        "--batch",
...
def main() -> None:
    ...
    for result, img_path in zip(results, to_process):
        ...
        if result.boxes is not None and len(result.boxes) > 0:
            ...
            for box, cls in zip(boxes_xywhn, classes):
                ...
                # FIX 3: skip tiny detections (likely noise or very distant swimmers)
        ...
        # FIX 4: only write label file if we found valid detections
        # Empty files = "no swimmer here" — only want that if truly empty frame
        if lines:
            with open(label_path, "w", encoding="utf-8") as f:
                f.write("\n".join(lines) + "\n")
            written_labels += 1
        else:
            # No detection → don't write file, review these frames manually
    ...
    print(f"Skipped (no detection):    {skipped_no_detection}  ← review these manually")
    print(f"Skipped (too tiny):        {skipped_tiny}")
    print(f"Confidence threshold used: {args.conf}")
    print(
        f"\nTip: Check the {skipped_no_detection} undetected frames manually in "
        f"a tool like LabelImg or Roboflow — these are the hard cases your model "
        f"needs to learn from most."
    )
